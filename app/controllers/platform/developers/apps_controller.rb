class Platform::Developers::AppsController < Platform::BaseController
  before_filter :validate_current_developer

  def index
    @app = Platform::Application.find(params[:id]) if params[:id]
    @apps = Platform::Application.find_all_by_developer_id(Platform::Config.current_developer.id)
    unless @app
